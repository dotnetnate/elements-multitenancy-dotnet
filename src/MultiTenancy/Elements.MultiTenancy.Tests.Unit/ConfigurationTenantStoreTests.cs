...
public class ConfigurationTenantStoreTests
{
    ...
    [TestMethod]
    public void Constructor_When_Configuration_Is_Null_Then_Throws()
    {
        // Act & Assert
        Assert.ThrowsExactly<ArgumentNullException>(() => new ConfigurationTenantStore<TestConfiguration>(null!));
    }

    [TestMethod]
    public void Constructor_When_Tenants_Section_Is_Null_Then_Throws()
    {
        // Arrange
        var config = new ConfigurationBuilder().Build();

        // Act & Assert
        Assert.ThrowsExactly<ArgumentNullException>(() => new ConfigurationTenantStore<TestConfiguration>(config, null!));
    }

    [TestMethod]
    public async Task GetConfigurationAsync_When_TenantId_Is_Whitespace_Then_Throws()
    {
        // Arrange
        var config = new ConfigurationBuilder().Build();
        var store = new ConfigurationTenantStore<TestConfiguration>(config);
        ...
        // Act & Assert
        await Assert.ThrowsExactlyAsync<ArgumentException>(() => store.GetConfigurationAsync("   "));
    }

    [TestMethod]
    public async Task ExistsAsync_When_TenantId_Is_Whitespace_Then_Throws()
    {
        // Arrange
        var config = new ConfigurationBuilder().Build();
        var store = new ConfigurationTenantStore<TestConfiguration>(config);
        ...
        // Act & Assert
        await Assert.ThrowsExactlyAsync<ArgumentException>(() => store.ExistsAsync("   "));
    }
}