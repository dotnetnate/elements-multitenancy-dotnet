---
name: refresh-deps-pip
description: Internal worker that audits or updates a single python-pip project. Invoked by the refresh-deps orchestrator only.
user-invocable: false
disable-model-invocation: false
argument-hint: '<audit|update> --path <dir> --manifest <file> [--major] [--ignore <pkg,...>] [--skip-cache]'
---

# refresh-deps-pip — python-pip project worker

You audit or update **exactly one** pip-managed Python project (`requirements*.txt`). You are not user-facing.
...
pwsh -NoProfile -File .github/agents/refresh-deps/scripts/audit-project.ps1 `
  --path "<path>" --type python-pip --manifest "<manifest>" [--skip-cache] [--ignore <pkg,...>]
```
...
pwsh -NoProfile -File .github/agents/refresh-deps/scripts/update-project.ps1 `
  --path "<path>" --type python-pip --manifest "<manifest>" [--major] [--ignore <pkg,...>]
```
...
- CVEs: `pip-audit -f json`. If `pip-audit` is not installed, the script emits `tool: "pip-audit-missing"` — surface this as-is. Absence of CVE data is **not** a clean bill of health.
